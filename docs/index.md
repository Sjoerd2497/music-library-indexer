...
# API/Output
The output of the Music Library Indexer (MLI) is written to the disk.
The program creates snapshots, which capture the state of your 
music library at a point in time.

## JSON
The currently only supported form of a snapshot is a JSON file. The 
...
```
Picture type is a numeric linked to a list of values and descriptions
(defined [here](https://id3.org/id3v2.3.0#:~:text=Picture%20type%3A)): 
